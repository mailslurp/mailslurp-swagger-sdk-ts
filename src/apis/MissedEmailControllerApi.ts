...
export class MissedEmailControllerApi extends runtime.BaseAPI implements MissedEmailControllerApiInterface {
    ...
    async waitForNthMissedEmailRaw(requestParameters: WaitForNthMissedEmailRequest): Promise<runtime.ApiResponse<MissedEmail>> {
        if (requestParameters.inboxId === null || requestParameters.inboxId === undefined) {
            throw new runtime.RequiredError('inboxId','Required parameter requestParameters.inboxId was null or undefined when calling waitForNthMissedEmail.');
        }

        if (requestParameters.timeout === null || requestParameters.timeout === undefined) {
            throw new runtime.RequiredError('timeout','Required parameter requestParameters.timeout was null or undefined when calling waitForNthMissedEmail.');
        }
        ...
        const response = await this.request({
            path: `/missed-emails/waitForNthMissedEmail`.replace(`{${"inboxId"}}`, encodeURIComponent(String(requestParameters.inboxId))).replace(`{${"timeout"}}`, encodeURIComponent(String(requestParameters.timeout))),
            method: 'GET',
            headers: headerParameters,
            query: queryParameters,
        });
